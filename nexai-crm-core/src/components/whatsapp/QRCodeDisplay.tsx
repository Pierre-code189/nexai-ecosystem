...
import { Smartphone, CheckCircle2, ShieldCheck, Key, Check, Loader2 } from 'lucide-react';
    ...
    try {
      ...
      const unsubscribe = onSnapshot(statusDocRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (data.status === 'connected' && isAlive) {
            setGatewayStatus('connected');
          } else if (data.status === 'qr_ready' && isAlive) {
            setGatewayStatus('qr_ready');
          } else {
                ...
                {activeTab === 'qr' ? (
                  <>
                    <div className="p-3 bg-white rounded-2xl shadow-2xl border-2 border-slate-700 flex items-center justify-center min-h-[220px] min-w-[220px]">
                      {qrImageDataUrl && gatewayStatus === 'qr_ready' ? (
                        <img
                          src={qrImageDataUrl}
